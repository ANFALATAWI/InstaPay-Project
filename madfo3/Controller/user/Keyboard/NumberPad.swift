...
@IBDesignable open class NumberPad: UIView {
    ...
    @IBInspectable open var keyBorderWidth: CGFloat = 0.6 {
        didSet { updateKeys() }
    }
    
    @IBInspectable open var keyBorderColor: UIColor = .white {
        didSet { updateKeys() }
    }
    ...
    open var clearKeyIcon: UIImage? = UIImage(named: "backspace", in: Bundle.myBundle(), compatibleWith: nil) {
        didSet { updateKeys() }
    }
    
    open var clearKeyBackgroundColor: UIColor = defaultHighlightColor {
        didSet { updateKeys() }
    }
    ...
    open var customKeyText: String? = "here" {
        didSet { updateKeys() }
    }
    ...
    open var customKeyTitleColor: UIColor? = .white {
        didSet { updateKeys() }
    }
    
    open var customKeyBackgroundColor: UIColor? =  defaultBackgroundColor{
        didSet { updateKeys() }
    }
    
    open var customKeyHighlightColor: UIColor? = defaultHighlightColor {
        didSet { updateKeys() }
    }
    ...
    public static let defaultBackgroundColor: UIColor = UIColor(white: 0.4, alpha: 0.0)
    public static let defaultHighlightColor: UIColor = UIColor(white: 0, alpha: 0.4)
